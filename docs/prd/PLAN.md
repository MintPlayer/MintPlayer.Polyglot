...
## P30 — Build-time plugin auto-download from `pgconfig.json` — 🚧 planned (2026-07-16; issue #30, PRD `docs/prd/issue-30-plugin-autodownload/`, 3-agent investigation)
...
**Plan (one PR, slices as commits):** the `resolve → download → SRI-verify → zip-slip-safe extract →
`validateBackend` → cache → register` pipeline moves **into the exe** (npm registry HTTP API, abbreviated
...
- **Slice 3 — auto-download in `resolveConfiguredTargets`** (lock-first; in-box wins when the CLI version
  ...
  config generation, surfaced as diagnostics) + `install` rewrite with `--update`.
