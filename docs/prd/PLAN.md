...
- **Per-language emitter dialects (🚦 demand-gated, from issue #59, closed not-planned 2026-07-20).** One
  ...
  packaging/versioning decision, not a capability gap. **Not scheduled:** no consumer needs versioned
  dialects today and PRD §3 says don't build speculative capability. Revisit when a real consumer
  (e.g. a Unity or netstandard2.0 constraint the modern-C# emitter can't satisfy) demands it. NB: issue
...
`resolveConfiguredTargets` (`main.cpp:178-189` — the one choke point shared by `build`, watch, and the
LSP) resolves only in-box plugins, `file:` deps, and a cache filled by a **manual** `polyglot install`
(which shells out to `npm pack` + system `tar`, unversioned cache key, no integrity, no lockfile); any
versioned npm spec is rejected at build time. Consequence: the NuGet MSBuild integration hardcodes
`--target csharp` and MintPlayer.AI's TS twins are regenerated by hand — the stale-twin drift Polyglot
exists to eliminate. Full design + investigation: `docs/prd/issue-30-plugin-autodownload/{PRD,PLAN}.md`.

**As built (2026-07-16; one PR, slices as commits):** the `resolve → download → SRI-verify → zip-slip-safe
extract → `validateBackend` → cache → register` pipeline moves **into the exe** (npm registry HTTP API, abbreviated
packument, data-only — never npm lifecycle scripts; transport = WinHTTP / macOS libcurl / Linux
dlopen-libcurl→curl-subprocess ladder, no vendored TLS this milestone), with a minimal semver subset
(`^ ~ exact >= latest`, prerelease-excluded `maxSatisfying`), a committed **`pgconfig.lock.json`**
(`version/resolved/integrity` per package; lock-first ⇒ deterministic offline builds), a **versioned
cache** (`<cache>/<npm-name>/<version>/…` + `meta.json`, re-verified on load — §6.1's `registry.json`
global index is recorded as superseded), and `polyglot install` rewritten over the same pipeline (npm/tar
...
- **Slice 0 — testable units + primitives (CLI layer, Core stays IO-free).** `src/pkg/{sha,inflate,tar,
  ...
  reader (zip-slip-safe), `PgConfig`/`pluginCacheDir` extracted out of `main.cpp`'s anonymous namespace.
- **Slice 1 — semver subset + registry client** (`POLYGLOT_REGISTRY` → `.npmrc` → npmjs; `HTTPS_PROXY`/
  `NO_PROXY`; injected `HttpGet` so resolution is unit-testable offline).
- **Slice 2 — lockfile + versioned cache** (atomic writes, tamper ⇒ entry refused; clean cutover, old
  flat cache entries never consulted).
- **Slice 3 — auto-download in `resolveConfiguredTargets`** (lock-first; in-box wins when the CLI version
  satisfies the range — the P24 lockstep invariant supplies the in-box version; LSP failures memoized per
  config generation, surfaced as diagnostics) + `install` rewrite with `--update`. Post-plan finding from
  the gate: the lock's `resolved` URL is a hint, not the trust root — when it moves (registry migration),
  the pinned *version* re-locates via the current registry and the pinned *integrity* still gates
  acceptance, so a moved registry heals instead of bricking the lock.
- **Slice 4 — remove hardcoded fallbacks** (no-config + no `--target` ⇒ diagnostic; extensions from
  `fileExtension()` everywhere; capability-derived reference target).
- **Slice 5 — fake-registry script gate** (`tests/registry/run-registry.ps1`, Node fixture server built
...
SHA-512 integrity; a second build succeeds fully offline; a tampered tarball or cache entry is refused
with a clear diagnostic; the whole existing suite (conformance, MSBuild, watch) stays byte-identical.
