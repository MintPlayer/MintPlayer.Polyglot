...
// The TypeScript backend is DATA: the spec + rule JSON above, interpreted by the one shared emitter. (The
// ExternType/Bound member picks collapse into std overlays at P19 slice 9.)
std::string emitTypeScript(const ir::Module& module) {
    InterpretedEmitter emitter(&typescriptSpec, tsExprRules(), &ir::ExternType::tsType, &ir::Bound::tsTemplate);
