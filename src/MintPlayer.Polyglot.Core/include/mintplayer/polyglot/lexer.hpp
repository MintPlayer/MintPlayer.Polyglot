...
// `fileId` stamps every token's SourcePos (default 0 = the single/primary file); the module loader assigns
// distinct ids per loaded `.pg` so positions stay unambiguous across files (§4.8).
std::vector<Token> lex(const std::string& source, DiagnosticBag& diags, int fileId = 0);
