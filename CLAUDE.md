...
| Which **`.pg` lines ran in a CONSUMER's** coverage run? | `--origin-info` (P38, issue #69) — C# `#line` → PDB sequence points; TS/Python/PHP → a v3 sidecar (P39). Opt-in; off = byte-identical output |
| How do those reports become **one `.pg` number** across targets? | `polyglot coverage remap` (P39, issue #71) — reads cobertura/lcov/istanbul/clover, projects generated lines onto `.pg` lines, writes the same formats. Branch data is emitted **count-only**; see `docs/coverage.md` |
